...
<div
  class="flex flex-col items-center justify-center h-full gap-4 bg-base-100 p-6"
>
  <h2 class="text-2xl font-bold">Create a Room</h2>

  <fieldset
    class="fieldset bg-base-200 border-base-300 rounded-box w-80 border p-4"
  >
    <label class="label mt-2" for="max-players">
    ...
    <label class="label mt-2" for="word-selection-size">
      <span class="label-text">Word Choices</span>
    </label>
    <select
      id="word-selection-size"
      class="select select-bordered select-sm w-full"
      bind:value={wordSelectionSize}
    >
      <option value={3}>3 words</option>
      <option value={5}>5 words</option>
    </select>

    <label class="label mt-2" for="draw-timer">
    ...
    <label class="label mt-2" for="number-of-rounds">
  ...
  <div class="flex gap-4">
    <button class="btn btn-ghost" onclick={onBack} disabled={isSubmitting}
      >Back</button
    >
